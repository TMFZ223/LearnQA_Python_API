...
class Assertions:
    @staticmethod
    def assert_json_value_by_name(resoponse: Response, name, expected_value, error_message):
        try:
            response_as_dict = resoponse.json()
        except json.JSONDecodeError:
            assert False, f"Response id not in json format. response text is '{response.text}'"
